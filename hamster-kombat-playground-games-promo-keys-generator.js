...
const DEBUG = parseArg(
  ["debug"],
  (it) => (["true", "false", ""].includes(it) ? it !== "false" : null),
  false,
);
const TIMING_STRATEGY = parseArg(
  ["timing-strategy"],
  (it) => (["fastest", "realistic"].includes(it) ? it : null),
  "realistic",
);
const SERVER_ERROR_COOLDOWN = 300_000;
const SERVER_ERROR_RETRIES = 3;
const WITH_REINSTALL_TIME = true;
const DEVICE = parseArg(["d", "device"], (it) =>
  ["android", "ios"].includes(it) ? it : null,
);
const EXCLUDE = parseArg(
  ["e", "exclude"],
  (it) =>
    it
      .split(",")
...
const GAMES = {
  TRIM: async ({
    ...
    if (origin === "ios") {
      ...
      setup(
        "user-agent",
        "UnityPlayer/2021.3.17f1 (UnityWebRequest/1.0, libcurl/7.84.0-DEV)",
      );
    }
  ...
  RACE: async ({
    ...
    if (origin === "ios") {
      setup(
        "user-agent",
        "Truckbountyhole/12 CFNetwork/1498.700.2 Darwin/23.6.0",
      );
    } else {
      setup(
        "user-agent",
        "UnityPlayer/2020.3.18f1 (UnityWebRequest/1.0, libcurl/8.5.0-DEV)",
      );
    }
  ...
  MERGE: async ({
    ...
    if (origin === "ios") {
      ...
      setup(
        "user-agent",
        "Mozilla/5.0 (Linux; Android 12; SM-S9110 Build/W528JS; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/95.0.4638.74 Mobile Safari/537.36",
      );
    }
  ...
  CLONE: async ({
    ...
    if (origin === "ios") {
      ...
      setup(
        "user-agent",
        "UnityPlayer/2022.3.25f1 (UnityWebRequest/1.0, libcurl/8.5.0-DEV)",
      );
    }
...
function globalId(type) {
  switch (type) {
    case "rand32": {
      return Array.from(crypto.getRandomValues(new Uint8Array(16)), (it) =>
...
class GamePromo {
  ...
  async fetchApi(path, body = null, retry = 0) {
    ...
    try {
      ...
      if (retry < SERVER_ERROR_RETRIES) {
        console.error(
          "Received network error, will retry after cooldown period.",
        );
        debug(err);
    ...
    if (!res.ok) {
      ...
      if (retry < SERVER_ERROR_RETRIES) {
        console.error(
          "Received internal server error, will retry after cooldown period.",
        );
        await globalDelay(SERVER_ERROR_COOLDOWN);
        return this.fetchApi(path, body, retry + 1);
      }
  ...
  async eventFetch(data) {
    ...
    const payload =
      this.origin === "ios" ? { promoId, ...data } : { ...data, promoId };
...
// Keep the main function as it was originally
async function main() {
...
// Add a simple HTTP server to keep the app running
const server = http.createServer((req, res) => {
  res.writeHead(200, { "Content-Type": "text/plain" });
  res.end(
    "HamsterKombat Promo Generator is running. Check logs for generated codes.",
  );
});
...
server.listen(port, () => {
  ...
  // Run the main function to start generating codes
